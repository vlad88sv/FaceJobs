...
if (!empty($_POST['categoria']) && is_numeric($_POST['categoria']) && !empty($_POST['estado']) && in_array($_POST['estado'],array('agregar','eliminar')))
{
    if ($_POST['estado'] == 'eliminar')
    {
        ...
        $consulta = sprintf('DELETE FROM `empresa_categorias_perfil` WHERE ID_perfil=%s AND ID_empresa_categorias IN (SELECT ID_empresa_categorias FROM `empresa_categorias` WHERE ID_empresa_categorias=%s AND ID_cuenta=%s) LIMIT 1', $ID_CUENTA_PERFIL, $_POST['categoria'],usuario::$info['ID_cuenta']);
        ...
        $datos['ID_empresa_categorias'] = $_POST['categoria'];
...
if (!empty($_POST['crearCategoria']))
{
    ...
    $ID_empresa_categorias = db::insertar('empresa_categorias', $datos);
    
    unset($datos);
    
    $datos['ID_perfil'] = $ID_CUENTA_PERFIL;
    $datos['ID_empresa_categorias'] = $ID_empresa_categorias;
    db::insertar('empresa_categorias_perfil', $datos);
    unset($datos);
}

$consulta = sprintf('SELECT t1.`ID_empresa_categorias`, t1.`categoria`, (SELECT COUNT(*) FROM `empresa_categorias_perfil` AS t2 WHERE `ID_perfil`=%s AND t1.`ID_empresa_categorias` = t2.`ID_empresa_categorias`) AS "existe" FROM `empresa_categorias` AS t1 WHERE ID_cuenta=%s ORDER BY t1.`categoria` ASC', $ID_CUENTA_PERFIL, usuario::$info['ID_cuenta']);
$resultado = db::consultar($consulta);

while ($resultado && $registro = mysql_fetch_assoc($resultado))
{
    echo sprintf('<span class="categoria"><input type="checkbox" %s value="%s" name="categoria" id="categoria_%s" /><label for="categoria_%s">%s</label></span>',($registro['existe'] == '1' ? 'checked="checked"' : ''),$registro['ID_empresa_categorias'],$registro['ID_empresa_categorias'],$registro['ID_empresa_categorias'],$registro['categoria']);
}
?>